...
process generate_h5_tables {
	...
	~/gtrd/scripts/remapping/WASP/snp2h5/snp2h5 --chrom chrom_sizes.txt.gz \
		--format vcf \
...
process remap_bamfiles {
	...
	python3 ~/gtrd/scripts/remapping/WASP/mapping/find_intersecting_snps.py \
		--is_sorted \
		--output_dir \${PWD} \
		--snp_tab snp_tab.h5 \
		--snp_index snp_index.h5  \
	...
	bwa mem -t ${task.cpus} ${params.indexes} sorted.remap.fq.gz | samtools view -b > remapped.bam
	python3 ~/gtrd/scripts/remapping/bwa_script/filter_reads.py remapped.bam remapped.marked.bam ${nuclear_chroms}

	samtools sort -@ ${task.cpus} -l0 remapped.marked.bam | samtools view -b -F 512 - > remapped.marked.filtered.bam
	python3 ~/gtrd/scripts/remapping/WASP/mapping/filter_remapped_reads.py sorted.to.remap.bam remapped.marked.filtered.bam remapped.keep.bam
